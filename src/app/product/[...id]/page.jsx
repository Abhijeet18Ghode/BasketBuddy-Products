...
const Page = () => {
  ...
  const relatedProducts = useMemo(() => {
    if (productDetail && productDetail.category) {
      return products.filter(
        (p) => p.category === productDetail.category && p._id !== productDetail._id
      );
    }
    return [];
  ...
  const productsToShow = showMore ? relatedProducts : relatedProducts.slice(0, 5);
  ...
  if (!productDetail) {
    ...
    return (
      ...
      <div className="w-full   pl-5 pr-5 h-fit pb-5 flex flex-col gap-10 phone:pl-1 phone:pr-1">
        <Toaster />
  ...
  /> */}
    ...
    <div className="w-1/4 h-full flex laptop:flex-col desktop:flex-col desktopxl:flex-col  items-center justify-start gap-5 overflow-auto scrollbar-none desktop:gap-3 phone:w-full phone:flex phone:overflow-x-scroll phone:scrollbar-none phone:gap-2">
      ...
      {productDetail?.images?.map((image) => (
        <div key={image} className="w-32 h-32 overflow-hidden relative group phone:w-24 phone:h-24">
          <Image
            src={image}
            width={100}
            height={100}
  ...
  {productDetail?.variants?.length > 0 ? (
    productDetail.variants.map((variant) => (
      <div key={variant._id} className="flex flex-col gap-2">
        ...
        <h1 className="text-sm font-extrabold text-slate-400">
          Available: {variant.attributes.map(attr => attr.attributeType).join(', ')}
        </h1>
    ...
    <h1 className="text-xl font-extrabold text-slate-400 desktop:text-lg">No Variants Available</h1>
  )}
</div>
      ...
      <div className="w-full flex items-center justify-between desktop:mb-4">
        <div className="w-1/3 flex flex-col items-center justify-center">
          <Image src={"/logo/shipped.png"} width={50} height={50} alt="Shipping" className="desktop:w-10 desktop:h-10 phone:w-8"/>
          <h1 className="desktop:text-lg phone:text-lg">Shipping</h1>
          <p className="font-bold desktop:text-sm phone:text-sm">7 Days</p>
        </div>
        <div className="w-1/3 flex flex-col items-center justify-center">
          <Image src={"/logo/exchange.png"} width={50} height={50} alt="Return" className="desktop:w-10 desktop:h-10 phone:w-8"/>
          <h1 className="desktop:text-lg phone:text-lg">Return</h1>
          <p className="font-bold desktop:text-sm phone:text-sm">{productDetail.returnPolicy} Days</p>
        </div>
        <div className="w-1/3 flex flex-col items-center justify-center">
          <Image src={"/logo/guarantee.png"} width={50} height={50} alt="Warranty"className="desktop:w-10 desktop:h-10 phone:w-8" />
          <h1 className="desktop:text-lg phone:text-lg">Warranty</h1>
          <p className="font-bold desktop:text-sm phone:text-sm">{productDetail.warranty} Days</p>
        </div>
      </div>
        ...
        <div className="flex justify-center gap-10 mt-5 w-full phone:gap-5">
          ...
          >
             <Image src="/icons/info.png" width={35} height={35} className="phone:w-6 phone:h-6"/>
            Details
          </button>
          <button
          ...
          >
             <Image src="/icons/rating.png" width={35} height={35} className="phone:w-6 phone:h-6"/>
          ...
          >
             <Image src="/icons/discuss.png" width={35} height={35}  className="phone:w-6 phone:h-6"/>
            Discussion
          </button>
        </div>
    ...
    >
      ...
      <div className="flex flex-col items-start justify-between  w-full h-1/3 p-5 shadow-md rounded-lg border-x border-y overflow-auto scrollbar-none phone:p-2">
       <div className="flex items-center gap-3">
        <Image src="/icons/desc.png" width={35} height={35} className="phone:w-6 phone:h-6"/>
       <h2 className="text-2xl font-bold phone:text-sm">Description</h2>
       </div>
        <p className="text-lg phone:text-xs">{productDetail?.description}</p>
      </div>
   <div className="w-full flex items-center gap-5 h-1/3 phone:gap-5 phone:flex-col">
   {
    productDetail?.dimensions && <div className="flex flex-col items-start justify-between  w-1/2 h-full p-5 shadow-md rounded-lg border-x border-y phone:w-full phone:h-1/2 phone:p-2">
    <div className="flex items-center gap-3 ">
      <Image src="/icons/dim.png" width={35} height={35} className="phone:w-6 phone:h-6"/>
     <h2 className="text-2xl font-bold phone:text-sm">Product Dimensions</h2>
     </div>
      ...
      <div className="flex items-center gap-3 ">
        <Image src="/icons/ship.png" width={35} height={35} className="phone:w-6 phone:h-6"/>
       <h2 className="text-2xl font-bold phone:text-sm">Shipping Information</h2>
       </div>
  ...
  {productDetail?.specifications &&  <div className="w-full h-1/3 shadow-md border-x border-y rounded-lg p-5 flex flex-col items-start justify-between overflow-auto scrollbar-none phone:p-2">
   <div className="flex items-center  gap-3">
        <Image src="/icons/spec.png" width={35} height={35} className="phone:w-6 phone:h-6"/>
       <h2 className="text-2xl font-bold phone:text-sm">Specifications</h2>
       </div>
       {
        productDetail?.specifications?.map((spec)=>{
            return <h1 key={spec.key}  className="flex items-center gap-2 phone:text-xs"><span  className="text-lg font-bold text-slate-400 phone:text-sm">{spec.key}</span> : <span  className="text-lg phone:text-sm">{spec.value}</span></h1>
        })
       }
       </div>}
    </motion.div>
    ...
    >
      ...
      <div className="w-full flex flex-col gap-2">
        <div className="w-full flex items-center justify-between ">
          <h2 className="text-2xl font-bold phone:text-lg">Rating & Reviews</h2>
          <button
            onClick={toggleModal}
      ...
      {productDetail.reviews.length > 0 ? (
                ...
                <div className="flex items-center w-full justify-between">
                  <div className="flex items-center gap-3">
                    {review.reviewerImage ? (
                      <Image src={review.reviewerImage} width={100} height={100} />
                    ) : (
                    ...
                    <h1 className="text-xl font-bold phone:text-lg">{review.reviewerName}</h1>
                  </div>
                  <h1 className="flex items-center gap-1 justify-end">
                    <GoStarFill className="text-yellow-400 text-xl phone:text-lg" />
                    <h1 className="text-xl phone:text-lg">{review.rating}</h1>
                  </h1>
                </div>
        ...
        <div className="w-full h-3/4 flex flex-col items-center gap-5 justify-center">
          <Lottie loop={true} animationData={NoReviews} className="w-full h-full" />
          <button onClick={toggleModal} className="bg-blue-700 text-white font-extrabold p-3 rounded-lg">
            Make it Your First?
          </button>
        </div>
      )}
  ...
  {activeTab === "discussion" && (
    <motion.div
      ...
      transition={{ duration: 0.5 }}
    >
      <Lottie loop={true} animationData={ComingSoon} className="w-full h-full" />
    </motion.div>
  )}
</div>
      ...
      <d2iv className="w-full h-full flex flex-col gap-10">
      {/* Related Products Section */}
      <div className="flex flex-col gap-5">
        <div className="flex flex-col items-start gap-3 phone:gap-1">
          <h1 className="text-2xl font-bold desktop:text-lg phone:text-lg">Related Products</h1>
          <hr className="w-48" />
        </div>

       <div className="flex flex-col gap-10">
       {relatedProducts.length > 0 ? (
          <div className="w-full h-full flex flex-wrap items-center justify-start gap-2 phone:gap-0">
        ...
        {/* Show 'View More' button if there are more than 4 related products */}
        {relatedProducts.length > 4 && (
          <div className="flex justify-center  w-full">
            <button
              onClick={toggleShowMore}
              className={showMore ? "hidden" : `px-4 py-2   border-x border-y  w-full rounded-full `}
            >
              {!showMore && "Show More"}
            </button>
          </div>
