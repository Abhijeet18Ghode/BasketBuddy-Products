...
const Subnavbar = () => {
  ...
  const isHomePage = layout.includes(pathname) || pathname.startsWith("/category/");
        ...
        {loading ? (
          ...
          categories.map((cat) => (
            ...
            >
              ...
              >
                <Image src={`/icons/${cat?.name?.toLowerCase()}.png`} width={200} height={200} className='w-14 h-14  object-contain bg-cover desktop:w-12 desktop:h-12 phone:w-10 phone:h-10' />
                ...
                >
                  {cat.subcategories.map((subcategory) => (
                    <motion.span
                      ...
                      transition={{ duration: 0.3 }}
                      onClick={() => router.push(`/category/${cat.name.toLowerCase()}/${subcategory.name.toLowerCase().replace(/ /g, '-')}`)}
                    >
                      {subcategory.name}
                    </motion.span>
                  ))}
