...
const Navbar = () => {
  ...
  return (
    <div className="flex items-center w-full h-[7vh]  justify-between p-3 sticky top-0 z-50 bg-white">
      <Link href={"/"}  className="text-3xl desktop:text-xl phone:text-xl">
        eCart
      </Link>
      ...
      <div className="flex items-center w-2/5 relative phone:w-2/4">
        {/* Category Dropdown */}
        <select
          className="border border-slate-200 pl-5 pr-5 pt-2 pb-2 desktop:pt-1 desktop:pb-1  rounded-l-lg bg-white outline-none desktop:text-sm phone:hidden  phone:pr-1 phone:pt-1 phone:pb-1"
          value={category}
          onChange={(e) => setCategory(e.target.value)}
        >
          {categories.map((cat, index) => (
            <option key={index} value={cat} className='desktop:text-sm'>
              {cat}
            </option>
          ))}
        </select>
        ...
        <input
          ref={inputRef}
          className="outline-none border-t border-b border-r border-slate-200 pl-5 pr-5 pt-2 pb-2 rounded-r-lg w-full desktop:pt-1 desktop:pb-1 desktop:text-sm phone:text-sm phone:pl-1 phone:pr-1 phone:pt-1 phone:pb-1 phone:border-l phone:rounded-l-lg"
          placeholder="Search"
          value={keyword}
          onChange={(e) => setKeyword(e.target.value)}
          onFocus={() => setIsDropDown(true)} // Corrected to a function
        />
        <CiSearch onClick={submitHandler} className="-translate-x-10 text-3xl cursor-pointer desktop:-translate-x-7 phone:text-lg phone:-translate-x-7" />
        {isDropDown && (
          <div ref={dropdownRef} className='dropdown absolute top-12 left-0 w-full bg-white border rounded-lg shadow-lg z-50 max-h-80 overflow-y-auto scrollbar-none'>
            {/* Search History */}
      ...
      <div className="flex items-center justify-evenly text-2xl gap-5 desktop:text-lg desktop:gap-3 phone:text-lg phone:gap-2">
        ...
        <div className="relative">
          ...
          {isCartOpen && (
           <div ref={dropdownRef} className="absolute right-0 mt-2 w-96 bg-white border rounded-lg shadow-lg z-50 phone:w-64">
           <div className="p-3 flex flex-col  gap-2">
             {/* Remove the header and table design */}
             <div>
              <h1 className='text-xl font-bold phone:text-lg'>Your Products</h1>
             </div>
            <hr className='w-full'/>
             <ul className='p-2 flex flex-col gap-2 h-96 overflow-auto scrollbar-none'>
               {combinedCartItems?.slice(0,3).map((product) => (
                 <li key={product?.product?.id} className='flex items-center justify-between border-b border-slate-200 py-2'>
                   <Image src={product?.product?.thumbnail[0]} width={50} height={50} className='rounded' alt={product?.product?.title} />
                   <div className='flex flex-col items-start mx-2 flex-grow'>
                     <h1 className='text-lg font-semibold phone:text-sm'>{product?.product?.title}</h1>
                   ...
                   <div className='flex items-center gap-3'>
                     ...
                     <button className='text-lg' onClick={handleCartChange}>+</button>
                   </div>
                   <button className='text-lg text-red-500 ml-2' onClick={handleCartChange}>
                     <RiDeleteBin6Line />
                   </button>
                 </li>
               ))}
        ...
        <div className="relative">
          ...
          {isProfileOpen && (
            <div ref={dropdownRef} className="absolute right-0 mt-2 w-72 bg-white border rounded-lg shadow-lg z-50 phone:w-64">
              <div className="p-3 flex flex-col items-start gap-5">
                <div className='w-full flex flex-col gap-2'>
                <h3 className="font-bold text-lg">Hello, {session?.user?.name || 'Guest'}</h3>
                <hr className='w-full'/>
                </div>
                {session ? (
                  <div className='w-full'>
                    {
                      profileOptions.map((option)=>{
                        return <Link key={option.id}  href={`/profile/${option.option.toLowerCase().replace(/\s+/g, '')}`} className=" p-2 rounded-lg hover:bg-gray-100 text-lg w-full flex items-center gap-2 phone:text-sm ">
                       <span className='text-slate-400 phone:text-sm'>{option.logo}</span> {option.option}
                    </Link>
                      })
                    }
                    <button onClick={() => signOut()} className="flex items-center gap-2  p-2  w-full text-left text-sm text-red-500 font-semibold">
                    <IoIosLogOut /> Sign Out
                    </button>
                  </div>
                ) : (
                  <Link href="/user/login" className="block py-1 hover:bg-gray-100">
                    Sign in
                  </Link>
                )}
              </div>
