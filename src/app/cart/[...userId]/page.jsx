    ...
    if (cartData && cartData.cart) {
      const handleSetCartItems = () => {
        ...
        const combinedCartItems = cartData.cart.map((cartItem) => {
          ...
          ); // Find product by matching productId
          return {
            ...cartItem,
  ...
  const deleteProduct = async (productId) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const productToDelete = combinedCartItems.find(
        ...
        <div className="w-full h-[85vh]   flex flex-col  items-start justify-between  gap-5 bg-slate-100 p-5 rounded-2xl phone:h-auto ">
          <div className="flex items-center justify-between w-full ">
            <div className="flex flex-col items-start gap-1">
              <h1 className="text-3xl font-bold flex items-center gap-2 phone:text-lg">
                Shopping Bag
              </h1>
              <p className="text-lg text-slate-400 phone:text-sm">
                {cartData?.cart?.length} Products in your Bag
              </p>
            </div>
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={clearcart}
                className="flex items-center gap-2 text-white bg-red-500 p-2 rounded-lg phone:text-xs"
              >
                Clear Bag <AiFillDelete className="text-2xl phone:text-xs" />
              </button>
            </div>
          </div>
              ...
              {combinedCartItems?.length > 0 ? (
                  ...
                  <table className="w-full flex flex-col gap-5 h-full">
                    <thead className="w-full">
                      <tr className="w-full flex items-center justify-between">
                        <th className="w-1/3 text-center flex items-center justify-center phone:w-3/4 phone:text-xs">
                          Product
                        </th>
                        <th className="w-1/4 text-center flex items-center justify-center phone:hidden">
                          Brand
                        </th>
                        <th className="w-1/4 text-center flex items-center justify-center phone:text-xs">
                          Quantity
                        </th>
                        <th className="w-1/4 text-center flex items-center justify-center phone:hidden">
                          Price
                          ...
                          >
                            <td className="w-1/3 text-start flex items-center gap-5 phone:w-3/4 phone:flex-col">
                              <Image
                                ...
                                className="bg-slate-100 w-20 h-20 object-contain p-5 rounded-xl phone:w-10 phone:h-10 phone:p-2"
                              />
                              <Link
                            ...
                            <td className="w-1/4 text-center flex items-center justify-center gap-5 phone:gap-1">
                              <button
                                className="text-slate-400 text-lg border-x border-y p-1 rounded-sm phone:text-xs"
                                onClick={() =>
                                  handleDecrement(product?.product?._id)
                                }
                              >
                                <FaMinus />
                              </button>
                              <span className="text-xl phone:text-xs">
                                {product?.quantity}
                              </span>
                              <button
                                className="text-slate-400 text-lg border-x border-y p-1 rounded-sm phone:text-xs"
                                onClick={() =>
                                  handleIncrement(product?.product?._id)
                                }
                              >
                                <FaPlus />
                              </button>
                            ...
                            <td className="w-1/4 text-start flex items-center justify-center phone:hidden">
                              <span className="phone:text-xs">
                                Rs.{product?.product?.discountPrice}
                              </span>
                            </td>
                            <td className="w-1/4 flex items-center justify-center ">
                              <h1 className="font-bold phone:text-xs">
                ...
                <div className=" w-full flex flex-col items-center h-full justify-center gap-5 phone:gap-2 ">
                  <Lottie
                    ...
                    className="w-full h-96 phone:h-56"
                  />
                  <p className="text-xl font-bold phone:text-sm">
                    No Product in Your Cart
                  </p>
                  <Link
                    href={"/"}
                    className="bg-blue-700 text-white text-xl p-3 rounded-lg font-extrabold phone:text-sm phone:mb-2"
                  >
                    Explore Now
                  </Link>
                </div>
            ...
            <div className="w-1/4 flex flex-col gap-5 p-5 bg-white rounded-2xl phone:w-full">
              <div className="w-full flex flex-col items-start gap-3">
                ...
                <div className="flex items-center w-full justify-between gap-1">
                  <input
                    className="w-3/4 pt-2 pb-2 pl-3 pr-3 outline-none bg-transparent border-x border-y border-slate-300 rounded-lg"
                    placeholder="Type Here"
                  />
                  <button className="w-1/4 pt-2 pb-2 pl-3 pr-3 bg-gray-700 text-white rounded-lg ">
                    Apply
                  </button>
                </div>
              </div>
              ...
              <Link
                href={"/checkout"}
                className="bg-gray-700 w-full p-2 text-white font-bold rounded-lg text-center"
              >
